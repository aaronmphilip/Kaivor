...
- New payment flow: tap "New payment" ? type phone number or UPI ID ? tap matching contact ? enter amount Rs  optional note ? "Pay" button
    ...
    override suspend fun execute(context: SkillContext, params: Map<String, Any>): SkillResult {
        ...
        val goal = when (action) {
            "send", "pay" -> {
                ...
                val goal = """You are in Google Pay. Send Rs $amount to "$contact"${if (note.isNotBlank()) " with note \"$note\"" else ""}.
STEPS:
...
4. On the amount screen, enter Rs $amount using the number pad.$noteStep
$payStep. Tap the "Pay" button.
$stopStep. STOP at the UPI PIN entry screen - do NOT enter the PIN.
$reportStep. Report: recipient name confirmed as shown on screen, amount Rs $amount, PIN screen is ready.

NEVER enter the UPI PIN - this is a sensitive action
Always tap a suggestion from the list after typing the contact name. Never skip this step."""
