...
- Send Money flow: tap "Send Money", enter phone number or UPI ID in the search/recipient field, tap contact, enter amount, tap "Proceed to Pay"
- Amount entry screen: large rupee keypad; quick-select amount chips such as Rs 100, Rs 200, Rs 500; "Proceed to Pay" green button at the bottom
...
- Mobile Recharge flow: tap "Mobile Recharge", enter mobile number, operator auto-detected, browse plan cards, select plan, tap "Proceed to Pay"
- Transaction History: tap "History" tab; list of recent transactions with recipient/sender name, amount, date, and status
        ...
        val goal = when (action) {
            "send", "pay" -> {
                ...
                val goal = buildString {
                    appendLine("You are in PhonePe. Send Rs $amount to \"$upiId\"${if (note.isNotBlank()) " with note \"$note\"" else ""}.")
                    appendLine()
                    appendLine("STEPS:")
                    ...
                    appendLine("4. Enter the amount Rs $amount using the keypad.")
                    ...
                    appendLine("${payStep + 2}. Report: recipient name as shown on screen, amount Rs $amount, PIN screen is ready.")
                    appendLine()
                    append("NEVER enter the UPI PIN.")
                }.trimEnd()
            ...
            "recharge" -> {
                ...
                val goal = buildString {
                    appendLine("You are in PhonePe. Do a mobile recharge of Rs $amount for number \"$mobile\".")
                    appendLine()
                    appendLine("STEPS:")
                    ...
                    appendLine("5. Select the plan closest to Rs $amount. Look for the exact amount or the nearest match.")
