        ...
        val goal = when (action) {
            "book" -> {
                ...
                buildString {
                    ...
                    if (!useCurrentPickup) {
                        append("3. Tap INSIDE the PICKUP field (identified by its pickup placeholder above). Do NOT tap the small X/clear icon — tap the field text area itself.\n")
                        append("4. The pickup field is now focused. Type \"$pickup\". A suggestion list appears — tap the best match. WAIT for it to be selected before touching the destination field.\n")
                        append("5. Now tap INSIDE the DESTINATION field (identified by its destination placeholder). Type \"$destination\" and tap the best suggestion.\n")
                    } else {
