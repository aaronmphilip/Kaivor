...
class UberSkill : Skill {
    ...
    private fun buildUberGoal(
        ...
        if (book) {
            ...
            append("- Fare: Rs [read exact price from screen]\n")
            ...
            if (rideType.isNotBlank()) {
                append("$rideType: Rs [exact fare from screen] - ETA [exact ETA from screen]\n")
                append("(also list any other visible options below)\n")
            } else {
                append("- [ride type]: Rs [exact fare] - ETA [exact ETA] - one line per option\n")
            }
