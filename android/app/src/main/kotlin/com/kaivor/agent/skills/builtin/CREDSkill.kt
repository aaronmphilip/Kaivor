...
- Pay Bill flow: tap "Pay Bill" ? amount entry screen shows total due pre-filled; user can change amount Rs  tap "Proceed to Pay" ? UPI/NEFT selection screen ? STOP before OTP or UPI PIN entry.
        ...
        val goal = when (action) {
            "pay", "payment" -> {
                ...
                val payGoal = buildString {
                    appendLine("You are in CRED. Pay the credit card bill${if (card.isNotBlank()) " for the $card card" else ""}${if (amount.isNotBlank()) " of Rs $amount" else " (use total due)"}.")
                    appendLine()
                    appendLine("STEPS:")
                    ...
                    if (amount.isNotBlank()) {
                        appendLine("3. The amount field may show the total due - clear it and enter Rs $amount.")
                    } else {
                    ...
                    appendLine("6. Report: card name, amount Rs ${amount.ifBlank { "total due" }}, and that the PIN screen is ready for the user.")
                    appendLine()
