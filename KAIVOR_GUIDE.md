# Kaivor — The Complete Guide

---

## What Is Kaivor?

Kaivor is an AI agent that lives on your Android phone and does things for you. Not just answers questions — actually does them. It opens apps, taps buttons, fills in forms, reads the screen, and confirms when it's done. You send a message on Telegram (or WhatsApp), it executes on the phone, and you get a report back. Order food, book a cab, pay a bill, add a calendar event, search the web, play music, read a PDF — all by typing or speaking a sentence. No app-switching, no remembering which button to tap, no friction. Just describe what you want and Kaivor handles the rest.

---

## The Superpower — What Makes This Different

Every other AI assistant gives you answers. Kaivor gives you *actions*.

When you ask Siri or Google Assistant to "order biryani from Swiggy," it opens the Swiggy app and stops there. You still have to search, pick, add to cart, apply coupon, and pay. Kaivor actually completes the task — it reads the screen at each step, makes decisions, handles edge cases, and reports back when your order is placed.

It also runs 24/7 in the background without you having to unlock your phone. Someone sends you a WhatsApp message that needs a reply? Forward it to Telegram and say "reply with I'll be there by 8." Kaivor replies in WhatsApp for you. You're in a meeting and need to book a Rapido to pick up your kid? Text your Telegram bot. Done.

The key difference: Kaivor uses accessibility APIs to see and control every app on your phone — even apps with no API, no integration, no official automation support. If a human can tap it, Kaivor can tap it.

---

## Quick Start — First 5 Commands to Try After Setup

After completing setup and seeing the Kaivor dashboard, open Telegram and send these to your bot:
...
Telegram is how you talk to Kaivor. Your bot is private — only your account (the one you paired during setup) can control it. Nobody else can send commands.
...
| `/memory` | Review what Kaivor has learned about you |

---

### Voice Notes — Talk Instead of Type

Send a voice note to your Telegram bot and Kaivor will transcribe it and execute it as a command. No extra setup required.

This works the same as a text command — Kaivor listens, understands, acts.
...
**Example:** Hold the microphone in Telegram, say "Book a Rapido from home to office for now," send. Kaivor opens Rapido, sets the pickup and drop, and gets to the booking screen.
...
Quote a relayed notification in Telegram and say "reply: I'll call you back." Kaivor opens the originating app and sends the reply.
...
**To enable after setup:** Go to Kaivor app → Settings → Grant Notification Access.

---

### WhatsApp Channel — Dual-SIM Control

If you have a second SIM (or WhatsApp Business), you can control Kaivor from WhatsApp instead of (or in addition to) Telegram.

**Setup:**
1. Install WhatsApp or WhatsApp Business on the agent phone with a second number.
2. Enter that number in Kaivor settings under "WhatsApp Channel."
3. From your main WhatsApp, send commands to that second number.

Kaivor watches for messages arriving on that WhatsApp number and treats them as commands — exactly like Telegram messages.
...
When Kaivor is running a task, a small black pill appears on the screen — the "notch." It shows:
...
**To enable:** Grant "Display Over Other Apps" permission in Settings (shown during onboarding, or in Kaivor Settings).
...
Kaivor has one execution path: fast by default, visual when needed.
...
When you enable AI Call Answering (Settings → AI Call Answering) and set Kaivor as your default Phone app, every incoming call becomes managed automatically.

**Inbound flow:**
1. A call comes in while you're busy.
2. Kaivor answers after 2 rings using your ElevenLabs cloned voice.
...
This is the most powerful call feature. Instead of you calling someone, Kaivor calls them on your behalf and handles the entire conversation.
...
**What happens:**
1. Kaivor looks up the contact in your phone (or dials the number directly).
...
### App Skills — What Kaivor Can Do in Each App
...
Kaivor has two knowledge modes:
...
**Using a macro:**
Just send `bd` or `lunch` in Telegram — Kaivor expands it and executes the full command.
...
Kaivor automatically substitutes the full address when it sees a saved place name.
...
Shows your last 20 actions with timestamps, what was attempted, and whether it succeeded or failed. Use this to verify a task completed, debug something that went wrong, or just review what Kaivor did while you were away.

---

### Memory & Learning

Kaivor learns your preferences over time. When you consistently pick the same restaurant, route, or payment method — it remembers and applies those defaults automatically.
...
Send a PDF, image, or document file directly to the Telegram bot and ask Kaivor to work with it.
...
Kaivor can generate images using AI when connected to an image generation API.
...
*Note: Image generation requires a separate API key (e.g., OpenAI DALL-E or Stability AI). Configure it in Kaivor Settings → Image Generation.*
...
"Pay the electricity bill and then navigate to the gym" works. Kaivor executes them sequentially without you having to wait and send again.

**2. Reference previous context**
After ordering food: "How long will it take?" Kaivor knows you're asking about the order it just placed.

**3. Use conversational corrections**
If Kaivor picks the wrong restaurant: "No, not that one — I meant the one in Koramangala." It adjusts without you repeating the whole command.

**4. Ask for confirmation before paying**
In Ask Permission mode, Kaivor shows you the total before any payment action and waits for your `yes`. Useful when ordering from a new place.

**5. Send commands while the screen is off**
Kaivor runs in the background. Send the Telegram message from your laptop, it executes on the phone, and you get a report back — all without touching the phone.
...
Tap the × on the floating notch, or send `/stop` in Telegram. Kaivor will halt within 2–3 seconds and report what it had done so far.
...
For a new app or messy form, leave the relevant screen open. Kaivor will read the accessibility tree first and use visual reading automatically when the tree is not enough.

---

## For Business — Use Cases for Founders and Teams

Most Indian founders run their business from a single Android phone. Every vendor call, every client follow-up, every payment, every order, every calendar entry — it all happens on that one device, across a dozen apps, throughout a 14-hour day. Kaivor turns that phone into a leverage machine. One command to your Telegram does what would otherwise take five minutes of app-switching, scrolling, and typing.
...
### 1. Why Kaivor Is a Founder's Unfair Advantage

Every business action on an Android phone — sending a payment, booking a cab, placing a supplier order, reading an email, checking a calendar — is something Kaivor can do on your behalf, triggered by a single Telegram message.

The leverage comes from three compounding factors:

**Elimination of app-switching tax.** The average knowledge worker switches apps 300+ times per day. Every switch costs 20–60 seconds of reorientation. Kaivor collapses this to a single chat interface. You type one instruction, the phone executes it, you get a confirmation. You never leave Telegram.
...
**How it works.**
When a call comes in that you cannot answer, Kaivor answers it automatically using a cloned ElevenLabs voice — your voice, your name, your business. The caller hears a professional greeting. The AI listens, responds to basic queries, and takes a message. Within seconds of the call ending, you receive on Telegram:
...
Kaivor places the call, conducts the conversation in your voice, live-transcripts it to your Telegram, and if the other person asks something you need to answer personally — pauses the AI and asks you directly via Telegram. You type your answer, it relays it and continues. You get a full call summary the moment it ends.
...
Kaivor executes each payment sequentially, confirms each one in Telegram, and stops if a payment fails — giving you time to intervene before it cascades.

**WhatsApp vendor coordination.**
Most Indian suppliers, contractors, and delivery partners use WhatsApp. Kaivor can send structured messages to any of them without you composing anything manually:
...
Kaivor opens Sheets, navigates to the last row, and fills in each column. Use this after every business lunch, cab ride, or miscellaneous spend. By month-end, your expense sheet is complete without any reconciliation effort.
...
Most deals die because nobody followed up. Kaivor removes the friction:
...
The most powerful configuration for a small business is a dedicated Android phone running Kaivor full-time, separate from the owner's personal phone.
...
- Kaivor running 24/7 on that phone, always plugged in
- The owner's Telegram account connected to that Kaivor instance
...
**Cost of this setup:** The Android phone is a one-time cost. The SIM is ₹150–300/month. Kaivor subscription. ElevenLabs voice cloning for the greeting is a one-time setup. Total ongoing cost for the entire AI receptionist + automation layer: less than the salary of a part-time assistant for one day.
...
Kaivor reads, drafts, and sends Gmail without you opening the app.
...
**Kaivor stops responding after a few hours**
...
Make sure Accessibility Service is still enabled: Kaivor Settings → Accessibility → verify it's ON. Some phone updates reset this permission.

**Notification relay stopped working**
Check: Kaivor Settings → Notification Access → must be ON. Also ensure the app wasn't removed from the Notification Listener whitelist by a system update.

**"Overlay not granted" message in the notch**
Go to Settings → Apps → Kaivor → Display over other apps → Allow.

**Task finishes too quickly without completing**
The app UI may have changed or the target app may be blocking automation. Send the command again with the exact missing detail, or send a screenshot/document so Kaivor can use visual reading.
...
→ If Kaivor stops responding, see [Don't Kill Kaivor](DONT_KILL_KAIVOR.md) for phone-specific setup.
